...
namespace E_CommerceAR.Areas.AdminDashboard.Controllers
{
    ...
    public class ProductsController : BaseController
    {
        ...
        public async Task<IActionResult> SaveNewProducts (string Name ,
        string Category ,
        string Description ,
        int Price ,
        int OfferPercentage ,
        ...
        string Model)
        {
            try
            {
                ...
                var addproducts = new Products
                {
                    ...
                    //ColorValues = (string)ColorValues.ToList() ,
                    Model = Model ,
                    DealerId = HttpContext.Session.GetString("UserId")
                };

                List<Upload> uol = new List<Upload>();
                if (!string.IsNullOrEmpty(HttpContext.Session.GetString("Upload")))
                {
                    uol = JsonConvert.DeserializeObject<List<Upload>>(HttpContext.Session.GetString("Upload"));

                     addproducts.images = uol.Select(item =>
                        $"https://firebasestorage.googleapis.com/v0/b/finalprojectar-d85ea.appspot.com/o/products%2Fimages%2F{item.ImageName}?alt=media&token={_UserToken}")
                        .ToList();
                }
                ...
                {
                    string imageFolderName = $"products/images/";
                    string imageBucketName = "finalprojectar-d85ea.appspot.com";
                    var imageStorage = StorageClient.Create();
                    string imageFileName = $"{imageFolderName}{images.Name}";
